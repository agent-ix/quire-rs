...
mod tests {
    ...
    /// with zero corpus occurrences. The pre-port line-structural scanner's
    /// shared leading-annotation helper trimmed each preceding line and
    /// accepted any that began with `#`, indentation-blind, so it walked
    /// this comment into `h`'s span; the indentation-sensitive grammar keeps
    /// it contained in the block it visually belongs to.
    #[test]
