...
//! and it was false.
//!
//! `MODULE_MANIFEST_SCHEMA`, `SEMANTIC_CORE_VERSIONS`, `SEMANTIC_CORE_<version>`,
//! and `semantic_core_bundle` are all generated by `build.rs` at compile
//! time from published `@agent-ix` npm packages — never from a file
//! committed to this repository. `MODULE_MANIFEST_SCHEMA` comes from
//! `@agent-ix/semantic-schema`'s `semantic/v1/module-manifest.schema.json`;
//! the semantic-core bundles come from `@agent-ix/semantic-core` at each
//! pinned version.
