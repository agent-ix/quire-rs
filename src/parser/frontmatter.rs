...
/// consumers (e.g. the Filament extraction engine, FR-040) do not have to
...
pub fn extract_frontmatter(markdown: &str) -> FrontmatterResult {
    ...
    let map: Map<String, Value> = match parsed {
        ...
        // does not pollute Filament's index_errors (CR-010).
...
mod tests {
    ...
    fn malformed_yaml_returns_body_fallback() {
        ...
        // (CR-010): boundary consumers surface this as a parse failure.
    ...
    // FR-006-AC-7 / TC-658 (empty/null → Absent half of the status classification)
    #[test]
    fn empty_or_comment_only_frontmatter_is_absent_not_malformed() {
        ...
        // be reported as a parse failure (CR-010) — it is treated as absent.
        for input in [
            "---\n\n---\nbody",
            "---\n   \n---\nbody",
            "---\n# only a comment\n---\nbody",
