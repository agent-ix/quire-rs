...
| `unbacked_rows_total_all_repos` | 2,408 | 2,407 | **−1** |
...
## `unbacked_rows`: −1, fully resolved, zero residual

| Repo | Baseline unbacked | New unbacked | Δ | Baseline backed | New backed | Δ backed |
|---|---:|---:|---:|---:|---:|---:|
| quire-rs | 400 | 400 | **0** | 1,312 | 1,312 | **0** |
...
| **all repos** | **2,408** | **2,407** | **−1** | — | — | — |

**quire-rs: exact match, 0/0 — the earlier `+4`/`−3` was itself two review findings, not a residual span effect.** The first post-proptest-fix sweep (before independent review) showed quire-rs's own `unbacked_rows` at 404 against a baseline of 400, decomposed at the time to 4 precise newly-unbacked rows (`FR-002-AC-4` ×2, `FR-005-AC-4`, `FR-008-AC-3`) but left un-settled on the `−3 backed` half, timeboxed per explicit instruction. That gap is now closed: after fixing review findings F1 (`fuzz_target` scanning only the first top-level macro invocation) and F6 (`flat_leading_span_and_test`'s two-token attribute match missing the three-token inner-attribute shape, `#`, `!`, `[...]`), re-running the identical row-level comparison (`examples/plat843_unbacked_rows.rs`, kept for reproducibility — dumps `coverage::compute`'s `unbacked_rows` for a repo root against a module, run once per engine and diffed) over the same pinned `08d39ea2c...` tree shows **zero rows different, `unbacked_rows` 400/400 and `backed` 1,312/1,312 exact matches**. Which of F1/F6 closed it specifically was not isolated further — both touch leading-span/discovery logic in the same file, and the coordinator's own instruction ("either outcome is fine") did not require isolating the exact mechanism once the outcome itself was confirmed by direct, reproducible comparison rather than inferred.

**quire-protocol: `backed` +63, a real coverage gain, mechanism proven by source inspection.** The 14 `Function`→`TestFunction` reclassifications documented above (proven by example: `tc_011_assessment_bundle_refusals_are_typed_and_distinct`, PLAT-69's multi-line-attribute truncation causing a kind misclassification, not just a span error) are the direct cause: where a matrix criterion's declared reference type requires `TestFunction`-kind evidence specifically, a symbol previously misclassified as plain `Function` could never satisfy it; correctly classified, it now can. **The 63 backed rows are not individually enumerated here** — the sweep harness does not dump full `unbacked_rows`/`backed` record lists (only `unmatched_tags`/`non_binding_tags` are full dumps, per PLAT-840 F4), and reconstructing the per-row set would mean re-implementing `coverage::compute`'s internals outside the library. This was explicitly descoped by the coordinator once the mechanism was proven by direct source inspection on a named, representative example — that example's single `#[trace(...)]` carries 7 trace ids, which makes "63 backed rows from 14 reclassified symbols" arithmetically plausible on its face, consistent with the proven mechanism rather than merely not contradicting it.
