...
**quire-rs: `+4` unbacked, `−3` backed — partially decomposed, timeboxed.** quire-rs's own symbol list is byte-identical to the old engine (see above), so this shift is not a missing-or-extra-symbol effect; it must be a **span** effect. Attempted a direct row-level decomposition using a new tool, `examples/plat843_unbacked_rows.rs` (kept, reproducible: dumps `coverage::compute`'s `unbacked_rows` for a given repo root against a given module — run once per engine, diff the output, no reimplementation of `coverage::compute`'s internals needed):

- Ran against `/home/peter/dev/quire-rs` (the pristine `08d39ea2c...` tree) with both an old-engine and a new-engine build: 3,403 Rust symbols both sides (apples-to-apples confirmed), `unbacked_rows` 400 (old) vs 404 (new), matching the sweep exactly.
- Diffing the two row lists finds exactly **4 newly-unbacked rows**, precisely: `FR-002-AC-4` (twice — once as the FR's own verification-method row, once via `TC-002b` in `spec/tests.md`), `FR-005-AC-4`, `FR-008-AC-3`. Zero rows disappeared from the unbacked list (nothing moved the other direction within `unbacked_rows` itself).
- The `−3 backed` did **not** show up as 3 of those 4 rows simply "always having been unbacked before" — `backed`+`unbacked` isn't the whole `total` (1,712/1,713 of 1,799, leaving ~86-87 rows in another bucket such as `no_symbol_rows`), so a row can move between `backed` and that third bucket without ever appearing as a diff in the `unbacked_rows` list alone. Separating "3 rows that were backed and now aren't" from "rows that moved between other buckets" needs one more field dumped (`no_symbol_rows`/`undeclared_statuses`) that the tool doesn't currently print.
- Opened the source for all three trace ids (`src/validate.rs:408`, `src/parser/mod.rs:502`, `src/parser/slice.rs:175,229`, `src/corpus/unlinked.rs:550`): every mention is a **legacy body comment** (`// FR-002-AC-4: apply_patch never panics...`), not a `#[trace(...)]` attribute — this repo dogfoods the legacy textual-tag form (FR-051-CON-3) alongside the canonical one, which is a different binding path than the `#[trace(...)]` attribute case proven above for quire-protocol, and needs its own check rather than being assumed to follow the same mechanism.
- **Timeboxed and stopped here, per explicit instruction, rather than building a second dump field to keep going.** The 4 newly-unbacked rows are the full, precise account of the `+4`; the exact 3 rows behind `−3 backed`, and whether either span fix's window now starts too late for any of them (a real regression, not a fix, if so), is **not decomposed** — this is a known, named gap, not an asserted "no regression." A natural follow-up: extend `plat843_unbacked_rows.rs` to also dump `no_symbol_rows`/`undeclared_statuses` and diff those too, which would very likely settle it with the same technique in a few more minutes.
