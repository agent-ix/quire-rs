...
| 5 | **Default / fallback content** — supply text for a missing field | **Drop.** No generation step; a missing required section is an error, not a fill. | No archetype-supplied default body text. | None intended; the starter-boilerplate role moves to the per-archetype authoring **skeleton** (ADR 0004 / [FR-029](../../functional/FR-029-archetype-input-contract.md) recast). |
