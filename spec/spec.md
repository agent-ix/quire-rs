...
- Semantic declaration extraction — under a module's `semantic` block, the typed `## Properties` table or `sysml` fence, `## Invariants`, and `## Operations` of an object artifact extract to semantic-core `FieldDecl[]`, `ClauseRef[]`, and `OperationDecl[]` with source spans and explicit availability ([FR-069](./functional/FR-069-semantic-module-contract-at-load.md)..[FR-072](./functional/FR-072-semantic-extraction-surface.md), [NFR-021](./non-functional/NFR-021-semantic-extraction-boundary.md)). Fence bodies are carried verbatim; nothing is parsed, typechecked, rendered, or generated. Model features (generalization, abstract types, presence, subsetting, redefinition, operation contracts and effect frames, populations, and object-type sections) extract under `model` only where the module manifest declares them ([FR-075](./functional/FR-075-model-feature-extraction.md)). A `## Relationships` table extracts to `RelationDecl[]` checked against the FR-040 edge registry when the manifest names `relationships` ([FR-076](./functional/FR-076-relationships-extraction.md)).
...
- **Clause semantics, IR lifting, and code generation.** Quire records clause text and spans; typechecking OCL/SysML/FRETish (`agent-ix/quire-contract-ir#55`), lifting extraction into IR `typeDefinition`s (`agent-ix/filament-core-data#36`), SysML v2 emission (`#37`), and package publishing are compiler concerns. Quire never migrates a corpus repository: legacy Properties forms are reported, not rewritten.
