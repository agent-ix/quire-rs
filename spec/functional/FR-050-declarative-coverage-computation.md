...
| FR-050-AC-41 | A trace target MAY declare `required: false`; omission is exactly the existing required posture. An optional target whose archetype is absent emits no `archetype-matches-nothing`; an archetype-matching document that omits its optional section emits neither `section-matches-nothing` nor `section-holds-no-table` and does not enter the section-hit denominator. If the section is present, its table still mints rows and its id column is checked. An unknown/non-boolean posture fails module load, and the default serializes as it did before the field existed (#327). | Test (TC-1076) |
...
| FR-050-AC-43 | When a reference table has a status-shaped header but not its configured status column, coverage emits `status-column-matches-nothing` at the 1-based table-header line. A document reference MAY declare `status_column`; omission uses the model-wide `traceability.status.column` exactly as before. An override without a model-wide `traceability.status` vocabulary fails module load because it could classify no value. The record names the reference declaration, document, section, effective configured column and observed headers, states that status classification was skipped, and directs the author to align the configuration or rename the header without guessing which is intended. A table with the effective configured column remains quiet and status classification proceeds; a table with no status axis remains outside this check (#341). | Test (TC-1079) |
