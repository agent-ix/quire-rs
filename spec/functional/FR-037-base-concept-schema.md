...
afterward (FR-002 / FR-032 frontmatter validation), owns the rest of the
...
- **`validate_base_concept`** — validates against the base schema (`type`
  ...
  routed *by* its `type` — the corpus / bundle layer (FR-038).
- **`validate_concept_shape`** — validates the same field typing **without**
  requiring `type`. It is wired into `validate_document` (FR-032) so a document
  validated via an explicit `--archetype` override MAY legitimately carry no
  `type` and still pass, preserving the `quire-cli` FR-004-AC-5 behavior; its
