...
The grammar is `agent-ix/quoin` FR-104 (quoin PR #553, `31ca54d`); its golden
...
- The `RelationVocabulary` on the context: the extracting object type's
  name, the merged `edge_types` registry (verb → `{ category, inverse? }`),
  each object type's `roles`, and each object type's `allowed_links`
  (verb → target tokens).
- The `BundleIndex` `package` and `artifacts`: every artifact `id` of the
  bundle with its frontmatter `object` type, or none.
...
- The engine SHALL accept only the exact header columns `Name`, `Verb`,
  `Target`, `Multiplicity`, in that order. If the first block of the section is a table with any other
  header, or a list or fence, then the engine SHALL emit
  `semantic.feature-not-extractable` at the block's first line with `reason`
  `relationships`.
...
- The engine SHALL set `RelationDecl.verb` to the `Verb` cell and
  ...
  `true` exactly when the registry entry declares `inverse: part_of`,
...
| `Verb` is only a registry `inverse` label; the message names the forward verb and the target | `semantic.invalid-model-cell` | `inverse-verb` |
...
- While `semantic.mappings` names `relationships`, the engine SHALL report
  ...
  with a section that holds no block.
...
| FR-076-AC-6 | The cases `column-missing`, `column-extra`, `bullet-list`, `second-table`, and `second-section` yield their expected diagnostics with `availability.relations` `unavailable`. | Test |
| FR-076-AC-7 | The cases `mapping-not-declared`, `subset-header-unowned-section`, `preamble-table`, and `prose-without-mapping` yield their expected diagnostics and availability. | Test |
...
| FR-076-AC-9 | Every record of AC-1..AC-8 validates against `semantic-v1.schema.json`; every existing semantic case record is unchanged; `declaration_record` carries `relations` when extracted; an artifact without the token or a relationship-shaped table carries no relations keys (CON-1); renaming a verb while keeping its registry entry changes only `verb` (CON-2). | Test |

## Dependencies

- **Upstream**: [FR-040](./FR-040-object-edge-vocabulary.md), [FR-041](./FR-041-authorable-inverse-edges.md), [FR-069](./FR-069-semantic-module-contract-at-load.md), [FR-070](./FR-070-typed-properties-extraction.md), [FR-072](./FR-072-semantic-extraction-surface.md), [FR-075](./FR-075-model-feature-extraction.md); `agent-ix/quoin` FR-104 and its mapping fixtures at `31ca54d`
- **Downstream**: `agent-ix/filament-core-data#155` (`RelationDecl` `name` and `sourceSpan`), `agent-ix/filament-core-data#156` (`RelationDecl` lowering from extraction)
