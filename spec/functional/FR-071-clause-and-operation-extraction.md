...
| FR-071-AC-2 | The six `fence-` cases of `operations-cases.json` (semantic-core `0.3.0`), `fence-ocl-carried` among them, yield the recorded code, severity, column, message, and fence locus, and the recorded `clauses`, `clauseText`, and clause availability where present; `quire` yields no advisory. | Test |
