...
| FR-075-AC-13 | Under the published `@agent-ix/spec-objects-architecture@0.7.0` module, resolved as a dependency at test time (PLAT-901), `validate_document` of each of the `part`, `port`, `connection`, and `allocation` skeletons yields zero errors, and each declaration record carries exactly the keys its data schema requires. | Test |
