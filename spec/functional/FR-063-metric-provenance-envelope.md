...
| FR-063-AC-7 | The payload carries `minting.section_hit_rate` — the documents whose declared minting section was found, over the documents a required trace target's `archetype:` selected and its `exclude:` kept plus documents where an optional target's section was present — as a **ratio**. An absent optional section is outside the population; a corpus none of whose required minting sections were read is hollow and reported by name, while a partial read is not. A model declaring no `trace_targets` reports it as not computed with the condition named, never as zero. | Test (TC-1036, TC-1076) |
| FR-063-AC-8 | The payload carries `authoring.tag_rate` as tagged evidence symbols over candidate evidence symbols. Its `examined` and `matched` counts both equal the candidates classified as tagged or untagged, so zero authored tags is a measured zero rather than a hollow denominator. The metric method states that generic tag detection is independent of the declared trace grammar. | Test (TC-1060) |
