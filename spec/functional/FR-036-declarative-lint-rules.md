...
| FR-036-AC-7 | A `forbidden_section` rule (CR-020) produces exactly one finding `{rule, severity, message}` when the named section is **present** — the inverse of the other rule types, which only check a section that exists — and none when it is absent; the default message names the section, a custom `message` overrides it, severity mirrors the rule, `archetypes:` scoping applies as for the other types (a non-matching or unresolvable archetype runs it only when unfiltered), and a YAML round-trip preserves the `type: forbidden_section` discriminator. | Test (TC-764) |
