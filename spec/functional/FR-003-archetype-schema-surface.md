...
|----|----------|--------------|
| FR-003-AC-1 | `schema_for(registry, "fr")` returns the JSON Schema document loaded from `spec-artifacts-iso/spec_artifacts_iso/schemas/fr-frontmatter.schema.json` byte-identical to the source file (modulo `serde_json` normalization of key order — see NFR-006). | Test |
...
- **Upstream**: US-001, FR-013 (load-time schema compilation)
- **Downstream**: FR-002 (consumes the same compiled schema for validation)
