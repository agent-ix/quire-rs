...
> class. Issue #403 SHALL land before #413: both change the same `lex_line`
> pass, and #413 SHALL rebase on the accepted regex-state implementation and
> preserve TC-1836 through TC-1839 while replacing character materialization.
...
| FR-051-AC-28 | The unchanged `filament-core-data@3b75e01c652ba00bb07c352ff5467419401e792b` `test/compiler.test.ts` blob (SHA-256 `54c96129e59dab60b37f8ee3107ecd703aa8cf0f864279ff8f49b5be6402074a`) is readable to the TypeScript adapter and contains the test symbols around its brace-bearing regex literal. The demonstration retains the observed published default-branch head and proves the pinned revision is its ancestor; an unreachable revision does not satisfy this criterion even when a local blob has the expected digest. | Demonstration (TC-1839) |
...
is an operator. The exception is the matching outer `)` of an `if`, `for`,
`while`, or `with` control-flow head: it closes the head and begins the governed
statement context, where `/` may open a regex even when the statement is on the
same line. Nested parentheses inside the head do not receive that exception.
At file or statement start, or after an opening delimiter,
