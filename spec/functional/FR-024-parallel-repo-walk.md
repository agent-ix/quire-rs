...
| FR-024-AC-12 | `validate_bundle` bridges each walk `DocumentWithoutFrontmatter` diagnostic into exactly one `BundleReport` **warning** naming the path — in both postures, never an error and never affecting `is_valid()` — and the two flavors carry distinct machine reasons: `no-frontmatter` for an absent block, `malformed-frontmatter` for a block that is not a YAML mapping (CR-051). | Test (TC-820) |
