...
| US-019 Extract semantic declarations | Illustrative examples EX-1..4 | TC-1610 (EX-1), TC-1612 (EX-2), TC-1613 (EX-3), TC-1600 (EX-4) via FR-069/070 | ✅ Complete |
...
| FR-031 Unified archetype shape | AC-1..8 | TC-522 (validatable+extractable, no renderability), TC-523 (no body_extraction → extraction None), TC-524 (defaults retained), TC-525 (two validators), TC-526 (required_sections ignored+diag), TC-527 (resolve parity), TC-1871 (construct carried as-is), TC-1876 (construct `immutable` survives to construct()), TC-1877 (vendored `$defs/ConstructDeclaration` admits `immutable`) | 🚧 Pending implementation |
...
| FR-077 Trace search index (structural forward/inverse lookup) | AC-1..6; CON-1..3 | TC-1887..TC-1894 | ✅ Implemented |
...
| NFR-022 Current stable Rust qualification | declaration audit + real stable gate matrix + governed inspection | TC-1832..TC-1835 | ✅ Implemented and qualified on 1.98.1; SR-106, SR-107 |
...
| TC-750 | Repeated extraction over an identical fixture tree, at a pinned grammar version and feature set, emits byte-identical JSON and identical record ids | Property | P0 | FR-051-AC-10, NFR-006 | ✅ |
...
| TC-804 | A brace or a quote inside a raw string (`r#"…"#`), a lifetime (`&'a str`), a character literal or a nested block comment is content: the file is accepted and yields its symbols — the shape that made 33 of this repo's own source files yield zero symbols (CR-040). A string opened on one line carries to the next: a `//` or a brace on a continuation line is content, a raw string closes only on its matching hash count, and code after the closer is code again — the multi-line `r#"…"#` JSON fixture is what the 33 files held | Unit | P0 | FR-051-AC-15 | ✅ |
| TC-803 | One reading of a file serves every consumer: a brace inside a block comment, inside a carried template literal, and after an unterminated quote is content to all of them, the file is accepted on its real blocks alone, and the span a declaration ends at is its own (CR-039) | Unit | P0 | FR-051-AC-14 | ✅ |
...
| TC-1878 | A lowercase `object_types` archetype whose own id joins KIND and NUMBER with an underscore (`interface_004`, matching spec-objects-architecture's real `interface` object type and its 3-column `\| ID \| Criteria \| Verification \|` Acceptance Criteria table) registers as an obligation-bearing kind exactly like an FR does — one `trace_targets` entry plus one `obligations` source, no engine change. One AC backed, one not: both land in `coverage.totals`/`minted_targets` and neither in `untracked_symbols` (FR-053, agent-ix/quire-rs#460) | Integration | P0 | FR-053-AC-1 | ✅ |
...
| TC-1029 | A Python declaration embedded in a triple-quoted string — an assigned opener (`FIXTURE = """`), the `'''` kind, the `r`/`f`/`rb` prefixes and a same-line open-and-close — is structurally unreadable as a symbol under the tree-sitter walk, and the real `def` after all of them is still seen, confirmed against a naive line-scanning stand-in that does read the phantoms so the property is not vacuous (CR-115, #274; ported to the AST, PLAT-868) | Unit | P0 | FR-051-AC-20 | ✅ |
| TC-1030 | A triple delimiter inside a single-quoted string, escaped, or after a `#`, and a `#` inside a triple-quoted literal, never affect what the tree parses as a declaration — there is no adapter-owned scan state left to toggle incorrectly, tree-sitter's own lexer having resolved string/comment boundaries once as part of producing the tree (CR-115, #274; ported to the AST, PLAT-868) | Unit | P0 | FR-051-AC-20 | ✅ |
| TC-1031 | A declaration after an embedded string keeps its TRUE container: the tree gives each `class`/`def` its real parent directly, so there is no scope stack that could resume stale — the dominant mode by count, and the half a fix that only stopped the swallow would miss (CR-115, #274; ported to the AST, PLAT-868) | Unit | P0 | FR-051-AC-20 | ✅ |
...
| TC-1599 | A module with a valid `semantic` block and a digest-matching reference `data_schema` loads a `SemanticModule` record and resolved schema; `validate_document_in_registry` validates the declaration record against it | Integration | P0 | FR-069-AC-1 | ✅ |
| TC-1600 | `contract_version: 2.0.0` fails with `semantic.unsupported-contract-version` and no other semantic diagnostic; `semantic_core: 0.9.0` fails naming `0.9.0` and `0.3.0` | Unit | P0 | FR-069-AC-2 | ✅ |
| TC-1601 | An unknown block key, an undeclared export, `package: ix://agent-ix/x`, `targets: [go]`, and an export with an inline `data_schema` each fail with their named code and value | Unit | P0 | FR-069-AC-3 | ✅ |
| TC-1602 | Digest mismatch, missing file, non-JSON, no `$schema`, wrong `$id`, `..` and symlink escapes each fail with code, path, and reason; `{ schema, digest, type }` fails `semantic.data-schema-ambiguous` | Unit | P0 | FR-069-AC-4 | ✅ |
...
| TC-1606 | Every semantic-core version a `semantic` block may declare is a non-empty embedded bundle, sourced from the published `@agent-ix/semantic-core` package, every embedded file is a valid draft 2020-12 JSON Schema, and each version's bundle content digest matches its pinned `sha256:` value (`0.3.0` `65b4e8d…`), computed over the build-fetched bytes | Static | P0 | FR-069-AC-8 | ✅ |
| TC-1607 | Every default and fixture module without a `semantic` block loads to the archetype projection in `tests/fixtures/semantic/baseline/registry-archetypes.json` | Snapshot | P0 | FR-069-AC-9, FR-069-CON-3, FR-031-AC-7 | ✅ |
...
| TC-1610 | Vendored `config-version.table.md` extracts to the normalized `fields` of `config-version.expected.json`, every entry validating against `FieldDecl.json` | Unit | P0 | FR-070-AC-1 | ✅ |
| TC-1611 | Vendored `config-version.fence.md` extracts to the identical normalized array with `fieldsForm: fence` | Unit | P0 | FR-070-AC-2 | ✅ |
| TC-1612 | `both-forms.md` yields `fields` `unavailable` (`both-forms`) and `semantic.properties-both-forms` at the recorded fence line; table-then-bullets yields the same code at the list line; two `## Properties` headings yield `semantic.duplicate-section` | Unit | P0 | FR-070-AC-3 | ✅ |
| TC-1613 | Every `Type` case in `cell-cases.json` under `config-version.bundle.json` yields the expected target, `decimal`, `unit`, placeholder, or diagnostic; two matching names yield `semantic.ambiguous-type`; an empty index yields reason `no-bundle-index`; a non-`Identifier` token yields `semantic.invalid-type-token` | Unit | P0 | FR-070-AC-4 | ✅ |
| TC-1614 | Every `Multiplicity` case in `cell-cases.json` plus `*`, `0..*`, `0..0`, `2..2 unique`, `a..b` yields the expected `Multiplicity` or `semantic.invalid-multiplicity` at the row | Unit | P0 | FR-070-AC-5 | ✅ |
| TC-1615 | Every `Constraints` case in `cell-cases.json` plus `nullable`, `pattern: /a,b\|c/`, and a duplicated field name yields the expected constraints, flags, or diagnostic, with pattern text verbatim | Unit | P0 | FR-070-AC-6 | ✅ |
| TC-1616 | Every fence-line case in `cell-cases.json` yields `semantic.sysml-outside-subset` at the fence line; brace content of arbitrary UTF-8 text maps as the `Constraints` cell | Unit | P0 | FR-070-AC-7 | ✅ |
| TC-1617 | The pinned FR-006 copy, `legacy-bullets.md`, and `legacy-mixed.md` yield the form, line, and warning of `legacy.expected.json` with `properties` unchanged and `fields` `unavailable`; under `legacy_forms: error` the finding is an error | Unit | P0 | FR-070-AC-8 | ✅ |
| TC-1618 | An artifact of a module without a `semantic` block loads with no semantic digest or diagnostic, and the `properties` string is untouched (byte identity of the Filament record: TC-1632) | Snapshot | P0 | FR-070-AC-9, FR-070-CON-3 | ✅ |
...
| TC-1622 | Vendored `operations.md` (`quire` fences, semantic-core `0.3.0`) extracts to the `clauses`, `operations`, `clauseText`, availability, and empty diagnostics of `operations.expected.json`; `config-version.table.md` (`0.3.0`) yields its `immutable` `ocl` clause with the recorded span, the recorded `semantic.clause-language-unchecked` advisory, and lossy clause availability; each `clauseText` equals the fence body byte-for-byte | Unit | P0 | FR-071-AC-1 | ✅ |
| TC-1623 | The six `fence-` cases of `operations-cases.json` yield the recorded diagnostics, `clauses`, `clauseText`, and clause availability; `quire` yields no advisory under `0.3.0` | Unit | P0 | FR-071-AC-2 | ✅ |
...
| TC-1629 | For generated fence bodies of arbitrary UTF-8 text (backticks, tilde fences, longer closing runs, CRLF, nested shorter fences), `clauseText` equals the body bytes and `startLine`/`endLine`/`endColumn` match the fence lines | Property | P1 | FR-071-AC-6 | ✅ |
| TC-1630 | Every case in `tests/fixtures/semantic/cases.json` yields the expected declarations, `availability`, and ordered diagnostics through the library, and the record validates against `semantic-v1.schema.json`; every case carries `issue_ref` | Integration | P0 | FR-072-AC-1 | ✅ |
| TC-1631 | Fixtures exercise `available`, `not_applicable`, `missing`, `unavailable` per kind and both `lossy` values; each is a distinct JSON token and `unavailable` carries a non-empty `reason` | Unit | P0 | FR-072-AC-2 | ✅ |
| TC-1632 | A Filament snapshot with a `semantic` context yields `dataJson.semantic` and diagnostics with `locus` and mapped severity; without the context the result equals `tests/fixtures/semantic/baseline/filament-graph-cases.json` byte-for-byte | Snapshot | P0 | FR-072-AC-3, FR-072-CON-1 | ✅ |
| TC-1633 | A snapshot naming `contractVersion: 2.0.0` or `semanticCore: 0.9.0` is refused with the FR-069 code before any node is produced | Unit | P0 | FR-072-AC-4 | ✅ |
| TC-1634 | `validate_document` over the vendored corpus copy reports the legacy-form warning at its line and over `both-forms.md` the both-forms error at the fence line, with the library's codes; the error fails validation | Integration | P0 | FR-072-AC-5 | ✅ |
| TC-1635 | Under `make ci-python`, the Python binding's `extract_filament_core` and `extract_semantic` return JSON values equal to the Rust output for every semantic fixture case | Integration | P0 | FR-072-AC-6 | ✅ |
| TC-1636 | The WASM `extractFilamentCore` and `extractSemantic` return JSON values equal to the Rust output for every semantic fixture case — external: runs in `agent-ix/quire-wasm#3` CI, reported here by reference | Integration | P0 | FR-072-AC-6 | 🚧 external — agent-ix/quire-wasm#3 |
| TC-1637 | Two extractions over identical input serialize byte-identically; editing an unrelated section changes no declaration, span, or diagnostic | Property | P0 | FR-072-AC-7 | ✅ |
...
| TC-1842 | `Pre:`/`Post:`/`Modifies:`/`Creates:`/`Deletes:` fill `pre`/`post` and one `operationFrames` entry; a second `Pre:`, a dangling id, a second `Modifies:`, and an invalid frame path are refused and omit `model` | Unit | P0 | FR-075-AC-3 | ✅ |
...
| TC-1852 | The relocated `relationships.md` extracts to the `relations`, `relationSources`, and `availability.relations` of `relationships.expected.json` under its recorded context; each relation validates against semantic-core `0.3.0` `RelationDecl.json`; `relationships.expected.json` and `relationships-cases.json` both record `semanticCore` `0.3.0` and a non-empty `context.sources` (CR-182 drops only the vendored-`PROVENANCE.json`-sha256-pin clause, with no replacement; `context.sources` itself is still asserted) | Unit | P0 | FR-076-AC-1 | ✅ |
...
| TC-1859 | `good-and-bad-rows`, `two-error-lines`, `header-only`, `section-absent`, `first-failing-check-only` yield their availability; all 44 cases run, each emitting exactly `exactDiagnostics` diagnostics with a line `sourceSpan` | Unit | P0 | FR-076-AC-8 | ✅ |
...
| TC-1862 | The four `no-bundle-index*` cases other than `no-bundle-index-mixed-rows` (TC-1865): lowering rows carry the `no-bundle-index` advisory naming the target; a title with spaces and a bad multiplicity are refused with no advisory | Unit | P0 | FR-076-AC-11 | ✅ |
...
| TC-1865 | The vendored case `no-bundle-index-mixed-rows`: with no bundle index, a lowering row and a `many` multiplicity row yield only the `multiplicity` error at line 21 and `entry-errors: lines 21`; the advisory is dropped | Unit | P0 | FR-076-AC-14 | ✅ |
...
| TC-1876 | A manifest object type whose `construct` declares `immutable: true` (`$defs/ConstructDeclaration`, filament-core-service FR-035-AC-17) loads through the loader (which never validates `construct` against the vendored schema) and `CompiledArchetype::construct()` returns the whole `construct` value unchanged, `immutable: true` included; a sibling `construct` that omits `immutable` returns its whole value with no such key (`loader::tests::tc1876_construct_immutable_survives_to_construct`) | Unit | P0 | FR-031-AC-8 | ✅ |
| TC-1877 | `$defs/ConstructDeclaration`, compiled standalone out of the embedded `MODULE_MANIFEST_SCHEMA` bytes (revision `5b2af8b`) — what a manifest-validating consumer, e.g. the Python `validate_manifest` binding, checks a `construct` value against — admits `immutable: true` and still refuses an unrecognized key under `additionalProperties: false` (`tests/semantic_baseline.rs::tc1877_construct_declaration_schema_admits_immutable`) | Static | P0 | FR-031-AC-8 | ✅ |
| TC-1879 | A Rust test carrying `#[test]` anywhere in its attribute block — before `#[ignore]` and after it — is one test symbol either way, binding the same trace ids, with the `#[test]`-last spelling as the control (`agent-ix/quire-rs#387`) | Unit | P0 | FR-051-AC-25 | ✅ |
| TC-1880 | A Python `@pytest.mark.trace` `black` has wrapped across lines, one separated from its `def` by a second wrapped decorator, and a `def` whose signature spans lines each bind exactly the ids their single-line adjacent spelling binds, with that spelling as the control (`agent-ix/quire-rs#395`, `#459`) | Unit | P0 | FR-051-AC-25 | ✅ |
...
| TC-1922 | A `const`/`let`/`var` declarator whose value is a parenthesized *non-arrow* expression (`const sum = (a + b);`) mints no symbol — the 76-row false-positive class the PLAT-882 differential names (PLAT-882 PR #481 review finding F2). Does **not** kill mutation E5 (widening the `arrow_function` kind check alone): a `parenthesized_expression` carries no `parameters` field regardless of content, so `mint_arrow_const_declarators`'s second guard rejects this fixture on its own either way — see TC-1925 for the isolating case (PLAT-882 PR #481 review round 2). | Unit | P0 | FR-051-AC-1 | ✅ |
| TC-1923 | An `interface`'s `method_signature` member mints no symbol, and neither does the interface declaration itself — the 19-row false-positive class the PLAT-882 differential names (PLAT-882 PR #481 review finding F2/F5, mutation E6; the declaration-itself half added round 3, F5, closing a surviving mutation that added `interface_declaration` to `walk`'s `class_declaration` arm) | Unit | P0 | FR-051-AC-1 | ✅ |
...
| TC-1925 | A `const`/`let`/`var` declarator whose value is a non-arrow `function_expression` or `generator_function` (`const f = function() {}`, `const g = function*() {}`) mints no symbol — the isolating case mutation E5 needed and TC-1922 could not provide, since a `parenthesized_expression` value has no `parameters` field either way and never distinguishes the two guards `mint_arrow_const_declarators` runs in sequence, while a function/generator expression does carry one and only the first guard's `arrow_function` check rejects it (PLAT-882 PR #481 review round 2, mutation E5-corrected) | Unit | P0 | FR-051-AC-1 | ✅ |
| TC-1926 | A leading comment reaches an `await`-wrapped registration exactly as it reaches the non-awaited form — `stmt_anchor` now climbs `await_expression` alongside `expression_statement`/`export_statement`, so `leading_line` no longer differs by one between `await it(...)` and `it(...)` under an identical leading comment (PLAT-882 PR #481 review round 3, F4; zero real occurrences in either measured corpus) | Unit | P0 | FR-051-AC-14 | ✅ |
...
| TC-1872 | `part`, `port`, `connection`, and `allocation` tables extract to typed entries with `SemanticId`s, `TypeRef`s, multiplicities, directions, and row spans; an empty or omitted end multiplicity is absent; allocation `<id>/<member>`, own-package `ix://`, and imported references resolve; unknown names, non-imported packages, and an allocation `<id>/<member>` whose `<id>` artifact does not declare the operation `<member>` (`semantic.unknown-reference`); wrong object types — a part `Owner` naming a `part`, `port`, `connection`, `allocation`, or a declarer with no declared type at all, a Port `Owner` or allocation `Target` naming a non-part, an allocation `Source` naming neither a part/port nor, by `<id>/<member>`, a type (refusing the same four systems-record kinds a part `Owner` refuses), and a self-owning part (`semantic.reference-kind-mismatch`) — unknown directions, hyphenated ids, an empty table, `<id>/<member>` as an owner or connection end, a second row, and a second systems table are refused with no `model`; a failing row carries no `no-bundle-index` advisory; the match key names the declared table; an undeclared `Source \| Target` table is refused as `allocation`; a record with two systems-model records errors from `declaration_record()`; `BundleIndex::from_documents` (corpus mode) derives `BundleArtifact.operations` itself from each document's own `## Operations` section via `clauses::declared_operation_names` — every declared heading regardless of a body error on that or any other operation in the section — and an allocation source resolved against that derived index lifts the same as against an explicitly supplied one; an allocation `Source`'s self-reference to its own id checks `<member>` against that same heading-derived list, on a document whose own object type passes the kind check, and is refused on the kind (not the operation) when it does not (`tests/semantic_systems.rs`, `src/semantic/target.rs`, `src/semantic/clauses.rs`) | Integration | P0 | FR-075-AC-12 | ✅ |
| TC-1873 | The published `@agent-ix/spec-objects-architecture@0.7.0` module, resolved at test time via the `spec-objects-architecture-fixture` crate (PLAT-901), validates its `part`, `port`, `connection`, and `allocation` skeletons with zero errors, and each declaration record carries exactly its schema's required keys (`tests/semantic_systems.rs`) | Integration | P0 | FR-075-AC-13 | ✅ |
| TC-1874 | A `Features` table interleaving one field and two operations yields `model.featureOrder` in row order with row spans; the declaration record carries `featureOrder` as the ordered names and satisfies the vendored `Interface.json`; `FeatureKind`'s cell spelling, `Display`, and serde form agree (`tests/semantic_features.rs`) | Integration | P0 | FR-075-AC-14 | ✅ |
| TC-1875 | A `Features` row naming no declared feature (`semantic.unknown-feature`), of the wrong kind (`semantic.feature-kind-mismatch`), with an unknown kind, with a non-Identifier name, or repeated, is refused at its row, and a declared field or operation with no row is `semantic.missing-feature` at the header; a wrong-kind row is not also missing; each omits `model`; with `operations` unavailable no row naming an operation is checked (`tests/semantic_features.rs`) | Integration | P0 | FR-075-AC-15 | ✅ |
...
| TC-1644 | The semantic fixture suite yields identical JSON values across repeated runs and between Rust and Python under `make ci-python`, including diagnostic order and loci; WASM external via `agent-ix/quire-wasm#3` | Integration | P0 | NFR-021-AC-4 | ✅ |
| TC-1645 | Two loaded modules with one `semantic.package` fail the later sorted root with `semantic.duplicate-package` naming both; an unprovided import warns `semantic.import-unresolved` and loads; a two-module import cycle fails both with `semantic.import-cycle` | Unit | P0 | FR-069-AC-10 | ✅ |
...
| TC-1850 | A `quire` fence under `0.3.0` extracts cleanly; an `ocl` fence under `0.3.0` and `0.1.0` is carried with `semantic.clause-language-unchecked` and a lossy kind; `quire` under `0.1.0` is `semantic.clause-language-invalid` | Unit | P0 | FR-071-AC-8 | ✅ |
...
| TC-960 | The fixture's negative shapes register nothing through `extract_tree`: a variable title, a registration with no callback argument, and an `iterate(` lookalike — absence, never a fallback name (CR-090; PLAT-882: a whitespace-split modifier chain is admitted now, see TC-961) | Integration | P1 | FR-051-AC-18 | ✅ |
| TC-961 | The widened grammar's edges are pinned one by one: whitespace before `(`, whitespace between curried groups, an unbounded `.modifier` chain, and (PLAT-882) whitespace before the `.` are admitted; an empty modifier and an identifier continuing past `test`/`it` stay outside (CR-090) | Unit | P0 | FR-051-AC-18 | ✅ |
...
| TC-976 | Typed profiles load from manifests and reject empty identity, zero thresholds and malformed acronym entries | Integration | P0 | FR-074-AC-7 | ✅ |
...
| TC-1887 | A forward query by exact trace id returns `SearchResult.verifies`/`.implements` (claims) as fields distinct from `.citations` (non-claims), never a boolean on one homogeneous list, over a fixture carrying one of each: a bound test, a marked-implements function, and a plain-comment citation of the same id, with the citation asserted absent from both claim fields and the claims asserted absent from citations (PLAT-844) | Unit | P0 | FR-077-AC-1 | ✅ |
...
| TC-1892 | Over a genuinely mixed-language fixture (a Rust claim and a Python claim, each asserted non-empty before any `.all()` check so an empty result cannot pass vacuously), `symbol_language()` + `language_confidence()` resolve `structural` for the Rust claim (AST-grounded, PLAT-843) and `line_heuristic` for the Python claim (pre-PLAT-851 line scanner) — no result presents uniform confidence across languages (PLAT-844) | Unit | P0 | FR-077-AC-5 | ✅ |
...
| FR-050-AC-39 | TC-1074 |
| FR-050-AC-40 | TC-1075 |
| FR-050-AC-41 | TC-1076 |
...
| FR-075-AC-14 | TC-1874 |
| FR-075-AC-15 | TC-1875 |
| FR-076-AC-1 | TC-1852 |
| FR-076-AC-2 | TC-1853 |
| FR-076-AC-3 | TC-1854 |
| FR-076-AC-4 | TC-1855 |
...
| FR-051-AC-7 | TC-747 |
| FR-051-AC-8 | TC-748 |
| FR-051-AC-9 | TC-749, TC-1884 |
| FR-051-AC-10 | TC-750 |
| FR-051-AC-11 | TC-753 |
| FR-051-AC-12 | TC-798, TC-799 |
| FR-051-AC-13 | TC-800 |
| FR-051-AC-14 | TC-803, TC-1924, TC-1926 |
| FR-051-AC-15 | TC-804 |
| FR-051-AC-16 | TC-806 |
...
| FR-051-AC-25 | TC-1879, TC-1880, TC-1881 |
| FR-051-AC-26 | TC-1882, TC-1883 |
| FR-052-AC-1 | TC-779 |
| FR-052-AC-2 | TC-780 |
| FR-052-AC-3 | TC-781 |
| FR-052-AC-4 | TC-782 |
