...
| FR-031 Unified archetype shape | AC-1..8 | TC-522 (validatable+extractable, no renderability), TC-523 (no body_extraction → extraction None), TC-524 (defaults retained), TC-525 (two validators), TC-526 (required_sections ignored+diag), TC-527 (resolve parity), TC-1871 (construct carried as-is), TC-1876 (construct `immutable` survives to construct()) | 🚧 Pending implementation |
...
| TC-1876 | A manifest object type whose `construct` declares `immutable: true` (`$defs/ConstructDeclaration`, filament-core-service FR-035-AC-17) loads against the re-vendored `module-manifest.schema.json` (revision `5b2af8b`) and `CompiledArchetype::construct()` returns `immutable: true` unchanged as part of the raw value; a sibling `construct` that omits `immutable` carries no such key (`loader::tests::tc1876_construct_immutable_survives_to_construct`) | Unit | P0 | FR-031-AC-8 | ✅ |
