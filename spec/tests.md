...
| TC-1600 | `contract_version: 2.0.0` fails with `semantic.unsupported-contract-version` and no other semantic diagnostic; `semantic_core: 0.9.0` fails naming `0.9.0` and `0.1.0` | Unit | P0 | FR-069-AC-2 | ✅ |
...
| TC-1606 | Every semantic-core version a `semantic` block may declare is a non-empty embedded bundle, sourced from the published `@agent-ix/semantic-core` package, every embedded file is a valid draft 2020-12 JSON Schema, and each version's bundle content digest matches its pinned `sha256:` value (`0.1.0` `dd33c88…`, `0.2.0` `ef79c5d…`), computed over the build-fetched bytes | Static | P0 | FR-069-AC-8 | ✅ |
...
| TC-1852 | The relocated `relationships.md` extracts to the `relations`, `relationSources`, and `availability.relations` of `relationships.expected.json` under its recorded context; each relation validates against semantic-core `0.2.0` `RelationDecl.json`; `relationships.expected.json` and `relationships-cases.json` both record `semanticCore` `0.2.0` and a non-empty `context.sources` (CR-182 drops only the vendored-`PROVENANCE.json`-sha256-pin clause, with no replacement; `context.sources` itself is still asserted) | Unit | P0 | FR-076-AC-1 | ✅ |
