...
| TC-1880 | Content is content: a file holding a `{n,m}` regex quantifier yields its symbols rather than being abandoned, and a trace id written inside a string or a comment binds only where a declared tag channel reads it (`agent-ix/quire-rs#424`) | Unit | P0 | FR-051-AC-25 | 🚧 |
...
| TC-1882 | Static boundary audit (TC-756 pattern, a `scripts/audits/*.sh` check run by `make audit-static`): the symbol-extraction path invokes no compiler, package manager, or build of the analysed tree and resolves no type | Static | P0 | FR-051-AC-26, FR-051-CON-1 | 🚧 |
