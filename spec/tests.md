...
| FR-047 Acceptance-criteria grammar | AC-1..12; CON-1 | TC-707 (shape classification), TC-708 (every non-empty cell segmented), TC-709 (non-singular + pair idiom), TC-710 (vague-response via lexicon), TC-711 (no-observable-outcome), TC-712 (binding), TC-713 (finding fields + routing), TC-714 (generic --summary prefix), TC-715 (PyO3 parity), TC-751 (non-canonical-shape steers GWT → EARS), TC-754 (fenced/blockquote skip in supplements), TC-757 (module-data observable verbs) | ✅ Implemented (CLI-surface AC-8 awaits EXT-3 `quire-cli`) |
...
| FR-050 Declarative coverage computation | AC-1..9; CON-1..2 | TC-732 (traceability model load), TC-733 (malformed/absent model), TC-734 (unbacked rows), TC-735 (status lies), TC-736 (untracked symbols), TC-737 (per-group counts), TC-738 (byte-identical output), TC-739 (non-ISO model), TC-740 (no model → diagnostic exit), TC-756 (CON-2 static boundary audit) | ✅ Implemented (AC-9 CLI exit awaits EXT-3 `quire-cli`) |
...
| TC-707 | An EARS-shaped `Criteria` cell classifies `ears`, a Given/When/Then cell classifies `given-when-then`, and a cell matching neither yields one `unclassifiable` finding | Unit | P0 | FR-047-AC-1 | ✅ |
...
| TC-711 | An outcome clause with no concrete-object signal and no observable-result verb yields `no-observable-outcome`; a returned value / emitted record / exit code yields none | Unit | P0 | FR-047-AC-5 | ✅ |
| TC-712 | The `ac` grammar runs only on the FR `Acceptance Criteria` `Criteria` column and `### <doc-id>-AC-N` supplements; Constraints cells and NFR Statements receive EARS findings only | Unit | P0 | FR-047-AC-6, FR-042 | ✅ |
...
| TC-714 | `quire validate --summary` histograms by the generic `[<grammar>:<check>]` prefix; a corpus with both `[ears:*]` and `[ac:*]` findings shows both | Integration | P0 | FR-047-AC-8 | 🚧 (engine API green (`summarize_findings`); CLI histogram wiring is EXT-3 `quire-cli`) |
...
| TC-720 | `quire validate --severity ears:vague-response=error` fails a vague-only document and overrides a conflicting manifest entry; repeated `--severity` entries in one invocation apply independently | Integration | P0 | FR-048-AC-5 | 🚧 (engine API green (`merge_severity_overrides`); CLI flag wiring is EXT-3 `quire-cli`) |
| TC-721 | `--strict` semantics unchanged: exits 1 on any warning with --strict, 0 for warning-only documents without it | Integration | P0 | FR-048-AC-6 | 🚧 (engine surface unchanged + asserted; CLI exit-code check is EXT-3 `quire-cli`) |
...
| TC-740 | With no declared traceability model, `quire coverage` exits non-zero with a diagnostic naming the missing declaration | Integration | P0 | FR-050-AC-9 | 🚧 (engine API green (`CoverageError::ModelUndeclared`); CLI exit is EXT-3 `quire-cli`) |
...
| TC-751 | A `given-when-then`-shaped cell yields one `non-canonical-shape` finding while still classifying `given-when-then` (other checks run on its `Then` clause); an EARS-shaped cell yields none | Unit | P0 | FR-047-AC-10 | ✅ |
...
| TC-755 | A malformed `--severity` entry (unknown level, missing `=`, unparseable `<grammar>:<check>` key) is rejected with a usage diagnostic and a non-zero exit before validation runs | Integration | P0 | FR-048-AC-10 | 🚧 (engine API green (rejects malformed entries); CLI usage exit is EXT-3 `quire-cli`) |
...
| TC-757 | A module `observable_verbs` registry merges first-wins over the built-in defaults (module-added verb suppresses `no-observable-outcome`); with no declaration the built-in default set applies unchanged | Unit | P0 | FR-047-AC-12, FR-043 | ✅ |
