...
| TC-1836 | A table-driven TypeScript lexer test covers regex literals containing escaped `{`, escaped `}`, both braces, `[^}]`, escaped `/`, comment-shaped text, flags, a regex-shaped template-literal fragment, and regex statements after the matching outer `)` of `if`/`for`/`while`/`with` heads with nested-parenthesis controls; arithmetic grouping plus division and `/=` controls keep following code braces visible. Removing regex state, treating division as regex, or treating a control-head closer as an ordinary expression closer fails the test. | Unit | P0 | FR-051-AC-25 | 🚧 |
| TC-1837 | An unterminated regex literal and unmatched closing/opening code braces each produce one `SymbolDiagnostic` carrying the source path, construct, and exact 1-based offending line; the unreadable file emits zero symbols while extraction of a readable sibling still succeeds. | Unit | P0 | FR-051-AC-26 | 🚧 |
| TC-1838 | The qa-corpus `typescript-regex-brace` failure/control pair moves from 0/1 to 1/1 TypeScript bindings without changing the control. Reverting regex masking restores the pending corpus failure. | Integration | P0 | FR-051-AC-27 | 🚧 |
| TC-1839 | After retaining the observed published default-branch head and proving `filament-core-data@3b75e01c652ba00bb07c352ff5467419401e792b` is its ancestor, `test/compiler.test.ts` parses from SHA-256 `54c96129e59dab60b37f8ee3107ecd703aa8cf0f864279ff8f49b5be6402074a` without source changes and retains test symbols around the brace-bearing regex at line 1291; an unreachable-revision control is refused. | Demonstration | P0 | FR-051-AC-28 | 🚧 |
...
| FR-051-AC-25 | TC-1836 |
| FR-051-AC-26 | TC-1837 |
| FR-051-AC-27 | TC-1838 |
| FR-051-AC-28 | TC-1839 |
| FR-052-AC-1 | TC-779 |
| FR-052-AC-2 | TC-780 |
| FR-052-AC-3 | TC-781 |
| FR-052-AC-4 | TC-782 |
