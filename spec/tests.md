...
| TC-1878 | Attachment decides a symbol and its trace ids, not page position: a Rust test carrying `#[test]` anywhere in its attribute block, a `@pytest.mark.trace` wrapped by `black`, a marker separated from its `def` by a second wrapped decorator, and a declaration whose signature spans lines each classify and bind exactly as their single-line adjacent spelling does; a file holding a `{n,m}` regex quantifier still yields its symbols, and a trace id written as string or comment data binds only through a declared tag channel (`agent-ix/quire-rs#387`, `#395`, `#424`, `#459`) | Unit | P0 | FR-051-AC-25 | 🚧 |
| TC-1879 | Extraction over a fixture tree that has never been compiled and whose declared dependencies are not installed yields that tree's symbols and relations, with a static audit showing no compiler, package-manager, or build-artifact invocation and no type resolution on the extraction path | Static | P0 | FR-051-AC-26, FR-051-CON-1 | 🚧 |
...
| FR-051-AC-25 | TC-1878 |
| FR-051-AC-26 | TC-1879 |
| FR-052-AC-1 | TC-779 |
| FR-052-AC-2 | TC-780 |
| FR-052-AC-3 | TC-781 |
| FR-052-AC-4 | TC-782 |
