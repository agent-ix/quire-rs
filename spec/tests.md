...
| TC-1076 | `required` defaults true and is omitted on serialization; `required: false` round-trips, suppresses an absent target archetype or section and its section-hit denominator entry, while a present section still mints and validates; a non-boolean value fails load (#327) | Integration | P0 | FR-050-AC-41 | ✅ |
...
| TC-1079 | A `Coverage Status` header against configured `Status` preserves the former skipped-classification state but emits `status-column-matches-nothing` at the header, naming configured and observed columns plus both safe change targets; restoring `Status` is quiet, an explicit per-reference `status_column: Coverage Status` is also quiet and restores status-lie detection without changing sibling tables, and the same override without a model-wide status vocabulary fails module load (#341) | Integration | P0 | FR-050-AC-43 | ✅ |
