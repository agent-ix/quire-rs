...
| FR-026 Intra-spec reference resolution | AC-1..13; CON-1 | TC-486 (frontmatter edge), TC-487 (ix:// edge), TC-488 (dangling), TC-489 (cross-spec dangling), TC-490 (bidirectional), TC-491 (target-id extraction), TC-492 (O(edges) proptest), TC-501 (dedup), TC-620 (rel-path edge/dangling), TC-621 (index/log excluded), TC-622 (dedup parity across sources), TC-880 (shared masker contract), TC-881 (quoted `ix://` is a mention), TC-882 (quoted relative link is a mention), TC-883 (single-doc surface agrees) | ✅ Complete |
...
| TC-880 | Shared code-span masker (`crate::markdown`): contents neutralized and delimiters kept, byte-length-preserving over multi-byte characters, fenced blocks (with and without an info string) masked, an inner single backtick does not close a fence, a longer run is content not a closer, an unbalanced run opens no span | Unit | P0 | FR-026-AC-12, FR-026-AC-13 | ✅ |
| TC-881 | An `ix://` inside a code span or a fenced block mints no edge stub and no dangling-reference diagnostic — including the bare `` `ix://` `` whose harvested target was a lone backtick (#89); a used `ix://` in the same document still resolves, byte-identical to the source | Unit | P0 | FR-026-AC-12 | ✅ |
| TC-882 | A relative-path Markdown link inside a code span or a fenced block mints no edge stub; after an unbalanced backtick run a real relative link is still harvested | Unit | P0 | FR-026-AC-13 | ✅ |
| TC-883 | `harvest_edges` (the single-document surface behind the Python binding) and `resolve` return the same target set for a document containing both a mention and a use — the mask is inside the harvest function, not at the call site | Unit | P0 | FR-026-CON-1 | ✅ |
...
| TC-856 | A payload carrying **every** optional key — no_symbol_rows, criteria, diagnostics, obligations, both totals pairs — validates, so the optional keys are covered by a payload that has them rather than only by one that omits them (FR-055) | Integration | P0 | FR-055-AC-3 | ✅ |
...
| FR-026-AC-10 | TC-621 |
| FR-026-AC-11 | TC-622 |
| FR-026-AC-12 | TC-880, TC-881 |
| FR-026-AC-13 | TC-880, TC-882 |
| FR-027-AC-1 | TC-493 |
| FR-027-AC-2 | TC-494 |
| FR-027-AC-3 | TC-495 |
| FR-027-AC-4 | TC-496 |
