...
| TC-797 | A declared model matching zero rows: `quire coverage` renders `0/0` distinctly and never as `100%`, and `--strict` exits non-zero on it — the state that made a wired gate pass vacuously (CR-029) | Integration | P0 | FR-050-AC-14 | ✅ (CLI behaviour in `quire-cli`; `tests/cli_coverage.rs`) |
