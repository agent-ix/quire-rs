...
| FR-031 Unified archetype shape | AC-1..7 | TC-522 (validatable+extractable, no renderability), TC-523 (no body_extraction → extraction None), TC-524 (defaults retained), TC-525 (two validators), TC-526 (required_sections ignored+diag), TC-527 (resolve parity), TC-1871 (construct carried as-is) | 🚧 Pending implementation |
