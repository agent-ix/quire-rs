...
| TC-1878 | A Rust test carrying `#[test]` anywhere in its attribute block — before `#[ignore]` and after it — is one test symbol either way, binding the same trace ids, with the `#[test]`-last spelling as the control (`agent-ix/quire-rs#387`) | Unit | P0 | FR-051-AC-25 | 🚧 |
| TC-1879 | A Python `@pytest.mark.trace` `black` has wrapped across lines, one separated from its `def` by a second wrapped decorator, and a `def` whose signature spans lines each bind exactly the ids their single-line adjacent spelling binds, with that spelling as the control (`agent-ix/quire-rs#395`, `#459`) | Unit | P0 | FR-051-AC-25 | 🚧 |
| TC-1880 | A brace inside a regular-expression literal is content: a file holding a `{n,m}` quantifier yields its symbols rather than being abandoned, with the same file minus the quantifier as the control (`agent-ix/quire-rs#424`) | Unit | P0 | FR-051-AC-25 | 🚧 |
| TC-1881 | Extraction over a fixture tree that has never been compiled, whose declared dependencies are not installed and for which no lockfile or build artifact exists, yields that tree's symbols and relations | Unit | P0 | FR-051-AC-26 | 🚧 |
| TC-1883 | The diagnostic an unparseable file yields names that file and the line its error sits at, so a reader is sent to the defect rather than to the tree; a file whose error sits on a different line names that line instead, and the tree's other files still extract (`PLAT-843` verification step 8) | Unit | P0 | FR-051-AC-9 | 🚧 |
| TC-1882 | Static boundary audit (TC-756 pattern, a `scripts/audits/*.sh` check run by `make audit-static`): the symbol-extraction path invokes no compiler, package manager, or build of the analysed tree and resolves no type, and the symbols it reports come from a grammar-driven parser library's tree rather than from a reader this crate hand-rolls | Static | P0 | FR-051-AC-26, FR-051-CON-1 | 🚧 |
...
| FR-051-AC-25 | TC-1878, TC-1879, TC-1880 |
| FR-051-AC-26 | TC-1881, TC-1882 |
| FR-052-AC-1 | TC-779 |
| FR-052-AC-2 | TC-780 |
| FR-052-AC-3 | TC-781 |
| FR-052-AC-4 | TC-782 |
