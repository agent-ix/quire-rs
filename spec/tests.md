...
| TC-1874 | A `Features` table interleaving one field and two operations yields `model.featureOrder` in row order with row spans; the declaration record carries `featureOrder` as the ordered names and satisfies the vendored `Interface.json` (`tests/semantic_features.rs`) | Integration | P0 | FR-075-AC-14 | ✅ |
| TC-1875 | A `Features` row naming no declared feature (`semantic.unknown-feature`), of the wrong kind (`semantic.feature-kind-mismatch`), with an unknown kind, or repeated, is refused at its row, and a declared feature with no row is `semantic.missing-feature` at the header; each omits `model` (`tests/semantic_features.rs`) | Integration | P0 | FR-075-AC-15 | ✅ |
