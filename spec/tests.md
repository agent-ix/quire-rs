...
| TC-943 | A curried `it.skipIf(cond)(…)` / `it.each([…])(…)` registration, and one whose title wraps onto a later line, each register a test symbol; a variable title, an `iterate(` lookalike and a literal past the window register nothing (CR-084) | Unit | P0 | FR-051-AC-18 | ✅ |
| TC-944 | A declared `source_exclude` glob removes the matching file's symbols; a non-matching glob leaves the extraction byte-identical; a start-anchored glob cannot reach a nested directory of the same name; and the key can only subtract — `spec/**` cannot un-exclude the document root (CR-085) | Unit | P0 | FR-050-AC-22 | ✅ |
| TC-945 | `source_exclude` patterns are compile-checked at module load and name the key when they fail; a model declaring only non-source paths still reads as undeclared (CR-085) | Unit | P1 | FR-050-AC-22 | ✅ |
...
| FR-050-AC-19 | TC-822 |
| FR-050-AC-20 | TC-824 |
| FR-050-AC-21 | TC-941, TC-942 |
| FR-050-AC-22 | TC-944, TC-945 |
| FR-051-AC-1 | TC-741 |
| FR-051-AC-2 | TC-742 |
| FR-051-AC-3 | TC-743 |
| FR-051-AC-4 | TC-744 |
