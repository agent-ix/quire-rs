...
| FR-047 Acceptance-criteria grammar | AC-1..9 | TC-707 (shape classification), TC-708 (every non-empty cell segmented), TC-709 (non-singular + pair idiom), TC-710 (vague-response via lexicon), TC-711 (no-observable-outcome), TC-712 (binding), TC-713 (finding fields + routing), TC-714 (generic --summary prefix), TC-715 (PyO3 parity) | 🚧 Pending implementation |
| FR-048 Per-check grammar severity | AC-1..8 | TC-716 (manifest registry + accessor), TC-717 (first-wins + DuplicateGrammarSeverity), TC-718 (per-check error routing), TC-719 (absent key → warning), TC-720 (--severity override), TC-721 (--strict unchanged), TC-722 (type-only all-default), TC-723 (malformed entry fails load) | 🚧 Pending implementation |
