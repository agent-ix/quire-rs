...
| TC-1018 | A case binding a variant module without naming a relaxation ticket is rejected, and one binding the vendored ecosystem module loads without naming one — a corpus whose manifest always matches cannot exhibit the defect stranding 3,514 TC ids (FR-065) | Unit | P0 | FR-065-AC-15, FR-065-AC-16 | ✅ |
| TC-1019 | Two runs of one case over unchanged input produce byte-identical result records (FR-065) | Integration | P0 | FR-065-AC-17 | ✅ |
...
| TC-1026 | `corpus.yaml`'s `emitted` and `forward` lists are checked against the engine, not maintained by hand — the day a forward ticket lands, its token becomes a literal in `src/` and the run fails until it moves to `emitted`, which is the same edit that makes every fixture waiting on it go green (CR-112, #300) | Integration | P0 | FR-065-AC-34, FR-065-AC-35 | ✅ |
...
| TC-1028 | A failure case's assertions SEPARATE its own input from its control's: graded against the control's payload, `expect.yaml` must produce at least one mismatch — a floor, not a proof: ten of eleven controlled fixtures are satisfiable by one incidental scalar. **That denominator is as of corpus `776a6b3` and has not been re-audited since** — measured with `bounds.py --json`, controlled failure cases were 11 there, 33 at `db55b05` and **35** at `3ff72c0`, so "ten of" describes eleven fixtures out of thirty-five and #285 itself moved the population by two (CR-123); re-running the audit is #301's. Every other rule in FR-065 is a predicate on the shape of a declaration, and five review rounds each removed one shape that was non-empty and meaningless before the next found another; this is a predicate on meaning (CR-114) | Integration | P0 | FR-065-AC-42 | ✅ |
| TC-1032 | A `regression` case — no control, not findable, pinning a LANDED ticket — is accepted, and an ECOSYSTEM-bound one credits its cell while a VARIANT-bound one credits none and reads `GAP` with a reason naming its `relaxation_ticket`. Added when the first Wave 3 fix landed and its fixture's control became equivalent to the case: a parser fix makes both spellings parse identically, so the pair has nothing left to separate and AC-42 rejects it, correctly (CR-116). The variant half is CR-123: `#285` left one `regression` pin on a variant, the assertion `continue`d past it, and the narrower of the two rules was the one nothing checked | Integration | P0 | FR-065-AC-43, FR-065-AC-44, FR-065-AC-45 | ✅ |
