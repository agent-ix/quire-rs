...
| TC-1872 | `part`, `port`, `connection`, and `allocation` tables extract to typed entries with `SemanticId`s, `TypeRef`s, multiplicities, directions, and row spans; an empty or omitted end multiplicity is absent; allocation `<id>/<member>`, own-package `ix://`, and imported references resolve; unknown names and non-imported packages (`semantic.unknown-reference`), wrong object types (`semantic.reference-kind-mismatch`), unknown directions, hyphenated ids, an empty table, `<id>/<member>` as an owner or connection end, a second row, and a second systems table are refused with no `model`; a failing row carries no `no-bundle-index` advisory; the match key names the declared table; an undeclared `Source \| Target` table is refused as `allocation` (`tests/semantic_systems.rs`) | Integration | P0 | FR-075-AC-12 | ✅ |
