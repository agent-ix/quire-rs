...
| TC-1030 | A triple delimiter inside a single-quoted string, escaped, or after a `#` opens nothing, and a `#` inside a triple-quoted literal ends nothing; asserted both on the minted symbols and on the reader's own answer for each line, so a pass cannot be two errors cancelling (CR-115, #274) | Unit | P0 | FR-051-AC-20 | ✅ |
