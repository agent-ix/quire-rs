...
| FR-052 Acceptance-criteria property classification | AC-1..13; CON-1..4 | TC-779 (universal decomposition, three spans), TC-780 (metamorphic idioms + fixed precedence), TC-781 (CR-017 mask parity), TC-782 (weak-only boundary → no spans), TC-783 (`Example`, not extractable, no finding), TC-784 (span/offset invariants), TC-785 (CON-1 `ac` finding stream unchanged), TC-786 (`property_idioms` first-wins merge), TC-787 (binding parity with `ac`), TC-788 (FR-050 rollup counts), TC-789 (PyO3 parity), TC-790 (CON-4 `extractable` registry-independent), TC-791 (closed structural signals without a registry) | 🚧 Pending implementation |
