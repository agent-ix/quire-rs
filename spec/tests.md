...
| TC-1606 | Every semantic-core version a `semantic` block may declare is a non-empty embedded bundle, sourced from the published `@agent-ix/semantic-core` package, and every embedded file is a valid draft 2020-12 JSON Schema | Static | P0 | FR-069-AC-8 | ✅ |
...
| TC-1852 | The first-party `relationships.md` extracts to the `relations`, `relationSources`, and `availability.relations` of `relationships.expected.json` under its recorded context; each relation validates against semantic-core `0.2.0` `RelationDecl.json`; `relationships.expected.json` and `relationships-cases.json` both record `semanticCore` `0.2.0` (CR-182 drops the vendored-sha256-pin `context.sources` clause with no replacement) | Unit | P0 | FR-076-AC-1 | ✅ |
