...
| TC-1852 | The vendored quoin `relationships.md` extracts to the `relations`, `relationSources`, and `availability.relations` of `relationships.expected.json` under its recorded context; each relation validates against semantic-core `0.2.0` `RelationDecl.json`; the vendored fixtures match their sha256 pins; `relationships.expected.json` and `relationships-cases.json` both record `semanticCore` `0.2.0` and a non-empty `context.sources` (quoin FR-104-CON-2) | Unit | P0 | FR-076-AC-1 | ✅ |
