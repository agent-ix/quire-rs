...
| FR-005 parse_document API | AC-1..6 | TC-001, TC-029, TC-812, TC-813 (CR-046 header/body tiers) | ✅ Complete |
...
| FR-024 Parallel repo walk (load_repo) | AC-1..11 | TC-470 (N files→N docs), TC-471 (malformed→diagnostic), TC-472 (gitignore), TC-473 (path-sorted determinism), TC-474 (symlink loop), TC-475 (id derivation), TC-476 (bad root), TC-455 (bench), TC-502 (no shared mutable state in the walk fan-out; named-exemption audit widened to OnceLock/OnceCell — CR-047), TC-807 (type-driven membership), TC-808 (glossary scanner agrees) | ✅ Complete |
...
| TC-809 | `extract_tree_excluding` never enters an excluded subtree — the document root passed by coverage yields no symbols from `spec/` — and an empty exclusion list extracts identically to `extract_tree` (CR-045) | Unit | P0 | FR-050-AC-17 | ✅ |
| TC-810 | `quire coverage --scope <repo>` derives the document root `<scope>/spec`: repo-root `README.md`/`CHANGELOG.md`/`plan/*.md` are never read as documents, and the minted-id set over a compliant repo is byte-identical to a pre-split run (CR-045) | Integration | P0 | FR-050-AC-17 | ✅ |
| TC-811 | A `--scope` with no `spec/` directory exits non-zero with a diagnostic naming the missing document root — no silent fallback to walking the scope (CR-045) | Integration | P0 | FR-050-AC-17 | ✅ |
...
| TC-813 | `parse_body` under a `parse_header` header equals `parse_document` byte-for-byte on BOM/CRLF/empty-body/no-heading fixtures and on arbitrary UTF-8 by proptest (CR-046) | Property | P0 | FR-005-AC-6 | ✅ |
| TC-814 | With the corpus at `<scope>/spec`, a `document: spec/tests.md` target mints when `validate_bundle` gets document root and reference root separately, and un-mints (reference dangles) when the roots are conflated (CR-045) | Unit | P0 | FR-049-AC-9 | ✅ |
...
| TC-816 | Two OS threads first-touch the same document's `body()` on the real std once-cell (TSAN lane target): equal parsed bodies, repeated access pointer-identical, zero bodies parsed before the touch and only the touched document after (CR-047) | Integration | P0 | FR-025-AC-8 | ✅ |
...
| TC-818 | Coverage over a corpus holding declared-archetype and undeclared-archetype documents parses the declared bodies and leaves the undeclared unmaterialised, with selection decided on the header tier and the report minting as before (CR-049) | Integration | P0 | FR-050-AC-18 | ✅ |
...
| TC-802 | `archetype` and `document` declared together scan both origins from one entry — the corpus documents of that archetype and the auxiliary file the corpus walk skips — and rows from both reconcile against the same target kind (CR-038) | Unit | P0 | FR-050-AC-15 | ✅ |
| TC-797 | A declared model matching zero rows: `quire coverage` renders `0/0` distinctly and never as `100%`, and `--strict` exits non-zero on it — the state that made a wired gate pass vacuously (CR-035) | Integration | P0 | FR-050-AC-14 | ✅ (CLI behaviour in `quire-cli`; `tests/cli_coverage.rs`) |
...
| FR-050-AC-13 | TC-788 |
| FR-050-AC-14 | TC-797 |
| FR-050-AC-15 | TC-801, TC-802 |
| FR-050-AC-16 | TC-805 |
| FR-050-AC-17 | TC-809, TC-810, TC-811 |
