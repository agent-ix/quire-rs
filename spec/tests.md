...
| TC-822 | A declared model that selects nothing reports why: an unreadable declared `document:` named per affected declaration with path and OS error, a declared `archetype:` no document has when the model minted nothing, and a model with no `trace_targets`; a healthy model reports none and omits the key; excluding every match of an archetype is not reported as a missing one; and the auxiliary document is read once however many declarations name it (CR-054) | Integration | P0 | FR-050-AC-19 | ✅ |
