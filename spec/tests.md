...
| US-017 Agent verifies coverage deterministically | Illustrative examples | TC-724..TC-750 (via FR-049/050/051) | 🚧 Pending implementation |
...
| FR-047 Acceptance-criteria grammar | AC-1..10; CON-1 | TC-707 (shape classification), TC-708 (every non-empty cell segmented), TC-709 (non-singular + pair idiom), TC-710 (vague-response via lexicon), TC-711 (no-observable-outcome), TC-712 (binding), TC-713 (finding fields + routing), TC-714 (generic --summary prefix), TC-715 (PyO3 parity), TC-751 (non-canonical-shape steers GWT → EARS) | 🚧 Pending implementation |
| FR-048 Per-check grammar severity | AC-1..9 | TC-716 (manifest registry + accessor), TC-717 (first-wins + DuplicateGrammarSeverity), TC-718 (per-check error routing), TC-719 (absent key → warning), TC-720 (--severity override), TC-721 (--strict unchanged), TC-722 (type-only all-default), TC-723 (malformed entry fails load), TC-752 (`off` suppresses a check entirely) | 🚧 Pending implementation |
| FR-049 Verification-reference integrity | AC-1..8 | TC-724 (resolved reference clean), TC-725 (dangling finding), TC-726 (posture degradation), TC-727 (model-driven pattern/column), TC-728 (auxiliary trace-source harvest), TC-729 (no model → no findings), TC-730 (multi-annotation cells), TC-731 (deterministic findings) | 🚧 Pending implementation |
| FR-050 Declarative coverage computation | AC-1..9; CON-1..2 | TC-732 (traceability model load), TC-733 (malformed/absent model), TC-734 (unbacked rows), TC-735 (status lies), TC-736 (untracked symbols), TC-737 (per-group counts), TC-738 (byte-identical output), TC-739 (non-ISO model), TC-740 (no model → diagnostic exit) | 🚧 Pending implementation |
| FR-051 Source symbol extraction with relations | AC-1..10; CON-1..2 | TC-741 (adapter symbol extraction), TC-742 (identity stability), TC-743 (test-symbol classification), TC-744 (tag contexts bind), TC-745 (tags are module data), TC-746 (duplicate-tag dedup), TC-747 (FR-045 record shapes), TC-748 (defined_in/contains edges), TC-749 (unparseable-file degradation), TC-750 (byte-identical repeat) | 🚧 Pending implementation |
...
| TC-714 | `quire validate --summary` histograms by the generic `[<grammar>:<check>]` prefix; a corpus with both `[ears:*]` and `[ac:*]` findings shows both | Integration | P0 | FR-047-AC-8 | 🚧 |
...
| TC-720 | `quire validate --severity ears:vague-response=error` fails a vague-only document and overrides a conflicting manifest entry | Integration | P0 | FR-048-AC-5 | 🚧 |
| TC-721 | `--strict` semantics unchanged: exits 1 on any warning with --strict, 0 for warning-only documents without it | Integration | P0 | FR-048-AC-6 | 🚧 |
...
| TC-740 | With no declared traceability model, `quire coverage` exits non-zero with a diagnostic naming the missing declaration | Integration | P0 | FR-050-AC-9 | 🚧 |
...
| TC-744 | Doc-comment bare id, `Trace:` line, line-comment id, and `tc`-prefixed test name each mint one `verifies` relation to the captured trace id | Unit | P0 | FR-051-AC-4 | 🚧 |
| TC-745 | A fixture model with a different id pattern binds by its own declaration; with no declared tag patterns zero `verifies` relations are minted | Unit | P0 | FR-051-AC-5 | 🚧 |
| TC-746 | A duplicate tag inside one symbol mints one `verifies` relation and one diagnostic | Unit | P0 | FR-051-AC-6 | 🚧 |
